...
class DomBuilder {
  ...
  text(value) {
    this.parent.appendChild(this.document.createTextNode(String(value)));
  }
}

// Runs a view's `static content()` into `host`, wiring up its outlets.
function buildContent(host, content) {
  const builder = new DomBuilder(host.ownerDocument);
...
function defineElement(name, ElementClass) {
  ...
  lumine.elements.define(name, ({ HTMLElement }) => {
    class RealmElement extends HTMLElement {
      constructor() {
        super();
      ...
      initializeContent() {
        if (this.colorsInitialized) return this;
        if (typeof ElementClass.content === "function") {
          buildContent(this, ElementClass.content);
        }
    ...
    copyPrototype(RealmElement.prototype, ElementClass.prototype);
    return RealmElement;
  });
...
function copyPrototype(target, source) {
  for (let prototype = source; prototype && prototype !== ColorsElement.prototype;) {
    for (const key of Reflect.ownKeys(prototype)) {
      if (key === "constructor" || Object.prototype.hasOwnProperty.call(target, key)) continue;
      Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(prototype, key));
    }
